...
int dirflag=0;		// For Cocktail Sort, to change direction at the ends
int count=1;		// For Cocktail Sort, to keep count of how many are sorted at the end
int k=0;			// To Switch from Welcome screen to Main Screen
int sorting=0;		// 1 if Sorted
char *sort_string[]={"Bubble Sort","Selection Sort","Insertion Sort","Cocktail Sort"};
...
{
	glColor3f(0,0,1);
	bitmap_output(190, 665, "SORTING ALGORITHM VISUALIZER",GLUT_BITMAP_TIMES_ROMAN_24);
	...
	bitmap_output(10, 625, "This program sorts a random set of numbers in ascending order displaying them graphically as ",GLUT_BITMAP_HELVETICA_12);
	bitmap_output(10, 605, "bars with varying height",GLUT_BITMAP_HELVETICA_12);
	...
	if (sorting == 0)	// if not sorting display menu
	{
		...
		bitmap_output(150, 475, *(sort_string+sort_count),GLUT_BITMAP_HELVETICA_18);
	}
...
void front()
{
	...
	bitmap_output(190, 475, "SORTING ALGORITHM VISUALIZER",GLUT_BITMAP_TIMES_ROMAN_24);
	glBegin(GL_LINE_LOOP);
		glVertex2f(180, 470);
		glVertex2f(480, 470);
	glEnd();
